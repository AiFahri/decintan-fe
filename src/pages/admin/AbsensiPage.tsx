...
const tabs = [
  { id: 'lokasi' as AbsensiTab, label: 'Lokasi', icon: <MapPin className="w-6 h-6" /> },
  { id: 'laporan' as AbsensiTab, label: 'Laporan', icon: <Clock className="w-6 h-6" /> },
  { id: 'pengaturan' as AbsensiTab, label: 'Pengaturan', icon: <Settings className="w-6 h-6" /> },
];
...
export default function AbsensiPage() {
  ...
  return (
    <DashboardLayout breadcrumbs={breadcrumbsMap[activeTab]}>
      ...
      <TabSwitcher tabs={tabs} activeTab={activeTab} onChange={(tabId) => setActiveTab(tabId as AbsensiTab)} />
